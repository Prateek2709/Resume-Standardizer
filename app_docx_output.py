...
# Phoenix OTEL tracing (optional, for LangSmith integration and better visibility into LLM calls)
if "phoenix_tracing_initialized" not in st.session_state:
    tracer_provider = register(
        project_name="Resume-Standardizer",
        endpoint=PHOENIX_COLLECTOR_ENDPOINT,
        auto_instrument=True,
        batch=False,
    )
    st.session_state.phoenix_tracing_initialized = True
...
TEMPLATE_DOCX = "templates/Saksoft_Template.docx"
NON_TEMPLATE_DOCX = "templates/Non_Saksoft_Template.docx"
...
def _remove_doc_decorations(doc: Document) -> None:
    """
    Remove headers, footers, page numbers, footer separator lines, and page borders.
    More aggressive than before: clears the entire header/footer XML story,
    not just plain paragraphs.
    """
    for section in doc.sections:
        ...
        for hdrftr in (section.header, section.footer):
            root = hdrftr._element

            # remove EVERYTHING inside header/footer (paragraphs, tables, shapes, lines, page numbers)
            for child in list(root):
                root.remove(child)

            # keep one empty paragraph so Word stays happy
            root.append(OxmlElement("w:p"))

        # remove page borders if present
...
def _trim_trailing_empty_paras(doc: Document) -> None:
    """Avoid blank last page due to trailing empty paragraphs."""
    while doc.paragraphs and not (doc.paragraphs[-1].text or "").strip():
        p = doc.paragraphs[-1]
...
def _remove_paragraph_border(p) -> None:
    pPr = p._p.pPr
    if pPr is None:
        return
...
def _unwrap_hyperlinks_in_paragraph(p) -> None:
    """
    Convert <w:hyperlink>...</w:hyperlink> into plain runs so text remains
    but hyperlink/clickability is removed.
    """
    hyperlink_elems = list(p._p.findall(".//" + qn("w:hyperlink")))
...
def _clean_section_headers_and_links(doc: Document) -> None:
    """
    1) Strip hyperlinks but keep the text.
    2) Remove border/underline decoration from section headers.
    """
    for p in _iter_body_paragraphs(doc):
        # remove hyperlinks anywhere in body
        _unwrap_hyperlinks_in_paragraph(p)

        txt = re.sub(r"\s+", " ", (p.text or "")).strip().upper()

        # remove decorative border / underline effect from section headers
        is_section_header_para = (
            txt in _SECTION_HEADERS
            or any(txt.startswith(h + " ") for h in _SECTION_HEADERS)
        )

        if is_section_header_para:
            _remove_paragraph_border(p)
            for r in p.runs:
                r.underline = False

        # if this paragraph contains email text, make sure it is plain-looking text
        if _EMAIL_RE.search(p.text or ""):
            for r in p.runs:
                if _EMAIL_RE.search(r.text or ""):
                    r.underline = False
                    try:
...
def _copy_para_with_runs(src_p, dst_doc: Document) -> None:
    """Copy paragraph text + basic formatting + alignment."""
    if not (src_p.text or "").strip():
        return

    dst_p = dst_doc.add_paragraph()
    dst_p.alignment = src_p.alignment

    pf_s = src_p.paragraph_format
    pf_d = dst_p.paragraph_format
    pf_d.left_indent = pf_s.left_indent
    pf_d.right_indent = pf_s.right_indent
    pf_d.first_line_indent = pf_s.first_line_indent
    pf_d.space_before = pf_s.space_before
    pf_d.space_after = pf_s.space_after
    pf_d.line_spacing = pf_s.line_spacing
    ...
    for r in src_p.runs:
        ...
        if r.font:
            ...
            if r.font.color:
                nr.font.color.rgb = getattr(r.font.color, "rgb", None)


def render_clean_docx_inplace_from_docx_bytes(docx_bytes: bytes, out_path: str) -> None:
    """
    Best possible 'Ctrl+A' clean export for DOCX:
    - keeps body content exactly as-is
    - strips hyperlinks but keeps email text
    - removes section-header border/underline decorations
    - removes headers/footers/page numbers/page borders/footer lines
    """
    src = Document(io.BytesIO(docx_bytes))

    # strip hyperlink clickability + section-header decorative borders
    _clean_section_headers_and_links(src)

    # remove headers/footers/page numbers/footer separator lines/page borders
    _remove_doc_decorations(src)

    # remove trailing blank paragraphs
    _trim_trailing_empty_paras(src)

    src.save(out_path)


def render_clean_docx_from_docx_bytes(docx_bytes: bytes, out_path: str) -> None:
    """DOCX input: produce a clean DOCX (no headers/footers/borders) but preserve body formatting."""
    src = Document(io.BytesIO(docx_bytes))
    out = Document()

    # Copy body paragraphs
    for p in src.paragraphs:
        _copy_para_with_runs(p, out)

    _remove_doc_decorations(out)
    _trim_trailing_empty_paras(out)
    out.save(out_path)


def render_clean_docx_from_text(text: str, out_path: str) -> None:
    """PDF fallback: create clean DOCX from plain extracted text."""
    out = Document()
    for line in (text or "").splitlines():
        out.add_paragraph(line.rstrip())
    _remove_doc_decorations(out)
    _trim_trailing_empty_paras(out)
    out.save(out_path)
...
def to_firm_json(base_json: dict, resume_text: str, run_dir: Path, extracted_full: dict | None = None) -> dict:
    """Convert parsed resume JSON + resume text into Saksoft firm-resume JSON for DOCX placeholders."""
    NOT_AVAILABLE_LOCAL = "Not available"
    resume_text = _truncate_for_llm(resume_text)

    # NOTE: This schema MUST match the placeholders used in Saksoft_Template.docx
    firm_schema = {
        "name": "",
    ...
    prompt = f"""
    You are mapping a FULL extracted resume JSON into Saksoft firm-resume JSON (template variables).
    ...
    response = _chat_completion_with_retry(
        ...
        temperature=0,
        run_dir=run_dir,
        call_name="to_firm_json() — map extracted data to Saksoft template JSON"
    )
...
logo_path = os.path.join("Saksoft-logo.png")
st.image(logo_path, width=250)

st.markdown("<H1 style='color: #e2a244; text-align: left;'>Resume Standardizer | Saksoft</H1>", unsafe_allow_html=True)
st.markdown("<h3 style='color: #0e3b79; text-align: left, font-size=30px;'>AI-Driven Resume Standardization for Saksoft</h3>", unsafe_allow_html=True)
st.markdown(
    "<p style='color: #0e3b79; text-align: left;'>"
    "Upload PDF/ Word Document resumes to extract structured candidate information and generate resumes aligned with Saksoft’s official formatting standards."
...
if uploaded_files:
    ...
    for i, uploaded_file in enumerate(uploaded_files):
        ...
        if file_id in st.session_state.processed:
            ...
            firm_docx_path = run_dir / f"Saksoft_{safe_stem}.docx"
            non_docx_path = run_dir / f"Non_Saksoft_{safe_stem}.docx"
            clean_docx_path = run_dir / f"Clean_{safe_stem}.docx"
            if firm_json_path.exists():
                try:
                    firm_json = json.loads(firm_json_path.read_text(encoding="utf-8"))
                    render_firm_resume(
            ...
            if firm_docx_path.exists():
                with open(firm_docx_path, "rb") as f:
                    st.download_button(
                        label=f"Download Saksoft Version (DOCX) - {firm_docx_path.name}",
                        data=f,
            ...
            if non_docx_path and non_docx_path.exists():
                with open(non_docx_path, "rb") as f:
                    st.download_button(
                        label=f"Download Non Saksoft Version (DOCX) - {non_docx_path.name}",
                        data=f,
            ...
            if clean_docx_path.exists():
                with open(clean_docx_path, "rb") as f:
                    st.download_button(
                        label=f"Download Clean Version (DOCX) - {clean_docx_path.name}",
                        data=f,
                        file_name=clean_docx_path.name,
                        mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                        key=f"clean_cached_{file_id}"
                    )
            continue
        ...
        # --- CLEAN output (Ctrl+A style) ---
        clean_docx_path = run_dir / f"Clean_{safe_stem}.docx"
        try:
            if file_ext == ".docx":
                render_clean_docx_inplace_from_docx_bytes(file_bytes, str(clean_docx_path))
            else:
                render_clean_docx_from_text(resume_text, str(clean_docx_path))
        except Exception as e:
            clean_docx_path = None
            st.warning(f"Clean DOCX generation failed for {uploaded_file.name}: {e}")
        ...
        try:
            ...
            # --- Non-Saksoft external version (DOCX) ---
            non_docx_path = run_dir / f"Non_Saksoft_{safe_stem}.docx"
            ...
            try:
                for p in list(run_dir.glob("FirmVersion_*.pdf")) + list(run_dir.glob("Saksoft_*.pdf")):
                    try:
                        p.unlink()
                    except Exception:
                        pass
        ...
        # Download Saksoft version (DOCX only)
        ...
        # Download Non-Saksoft version (DOCX only)
        if non_docx_path and non_docx_path.exists():
            with open(non_docx_path, "rb") as f:
                st.download_button(
                    label=f"Download Non Saksoft Version (DOCX) - {non_docx_path.name}",
        ...
        # Download Clean version (DOCX)
        if clean_docx_path and clean_docx_path.exists():
            with open(clean_docx_path, "rb") as f:
                st.download_button(
                    label=f"Download Clean Version (DOCX) - {clean_docx_path.name}",
                    data=f,
